<script lang="ts">
	...
	/** Sum of all workers' production skill values (points generated per work click). */
	const totalProd = $derived(
		workers.reduce((acc, w) => acc + w.userData.skills.production.total, 0)
	);

	/** Sum of all workers' hourly wages. */
	const totalWages = $derived(workers.reduce((acc, w) => acc + w.wage, 0));
...
<Card.Root class="col-span-2 flex h-full max-h-120 flex-col overflow-hidden xl:col-span-1">
	...
	<Card.Header>
		...
		<div class="grid grid-cols-3 gap-2 text-center sm:grid-cols-3">
			{#each [{ label: 'Daily Production', value: totalDailyProduction.toFixed(1), hint: 'Production points/day', icon: MdiPickaxe }, { label: 'Avg. Energy', value: (totalEnergy / workers.length).toFixed(0), hint: 'Average energy skill', icon: MdiLightningBolt }, { label: 'Daily Wages', value: estimatedDailyWage.toFixed(3), hint: 'Total wage cost/day', icon: MdiBitcoin }] as stat}
